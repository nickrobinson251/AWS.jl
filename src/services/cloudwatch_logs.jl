...
that have permission to write to the S3 bucket that you specify as the destination. This is
...
the same S3 bucket. To separate out log data for each export task, you can specify a prefix
to be used as the Amazon S3 key prefix for all exported objects. Exporting to S3 buckets
